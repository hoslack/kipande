...
{
	$cnn = mysql_query('select password,id from admins where username="'.mysql_real_escape_string($_COOKIE['username']).'"');
	$dn_cnn = mysql_fetch_array($cnn);
	if($dn_cnn['password']==$_COOKIE['password'] and mysql_num_rows($cnn)>0)
	{
