...
<div class="reports">
    ...
    <div class="submitform" >
        ...
        <fieldset class="searchfield">
        	...
        	<legend>Search Cards by Bank</legend>

        <form method="POST" action="cardsbybank.php">
...
if(isset($_POST['search']))
{
	...
	$qresult=mysqli_query($conn,"SELECT * FROM bankcards WHERE sprovider LIKE '%$searchq%' order by fname");

     $output = "<table id='listtable'>
<tr> 
<th>First Name</th><th>Middle Name</th><th>Last Name</th><th>Card Number</th> <th>Institution</th> <th>Centre</th>
<th>Status</th> 
</tr>";
	...
	if(mysqli_num_rows($qresult)!=0){
		...
		while($row=mysqli_fetch_array($qresult)){
            ...
            $institution=$row['sprovider'];
			$centre=$row['centre'];
			$status=$row['Status'];


    $output= $output."<tr> <td>" .$fname. "</td> <td> ".$mname."</td> <td> ".$lname. "</td> <td> ".$cardnumber. "</td> <td> ".$institution. "</td> <td> ".$centre."</td> <td> ".$status. "</td> </tr>";
		}
$output=$output."</table>";
